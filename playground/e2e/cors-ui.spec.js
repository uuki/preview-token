...
test.describe('CORS origins UI', () => {
    ...
    test('shows at least one origin input row', async ({ page }) => {
        const rows = page.locator('.wpt-origin-row');
        await expect(rows.first()).toBeVisible();
    });

    test('Add origin button appends a new input row', async ({ page }) => {
        const before = await page.locator('.wpt-origin-row').count();
        await page.getByRole('button', { name: /add origin/i }).click();
        const after = await page.locator('.wpt-origin-row').count();
    ...
    test('remove button clears last row instead of deleting it', async ({ page }) => {
        ...
        while (await page.locator('.wpt-origin-row').count() > 1) {
            await page.locator('.wpt-remove-origin').last().click();
        }
        const input = page.locator('.wpt-origin-row input').first();
        await input.fill('https://example.com');
        await page.locator('.wpt-remove-origin').first().click();
        await expect(input).toHaveValue('');
        await expect(page.locator('.wpt-origin-row')).toHaveCount(1);
    });
    ...
    test('saves multiple origins and reloads them as separate inputs', async ({ page }) => {
        ...
        // Fill first row
        await page.locator('.wpt-origin-row input').first().fill('http://localhost:4321');

        // Add and fill second row
        await page.getByRole('button', { name: /add origin/i }).click();
        await page.locator('.wpt-origin-row input').nth(1).fill('https://*.amplifyapp.com');

        await page.getByRole('button', { name: /save changes/i }).click();
        await page.waitForURL(`${WP}/wp-admin/options-general.php*`);

        const inputs = page.locator('.wpt-origin-row input');
