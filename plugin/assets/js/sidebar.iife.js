(function() {
	...
	*/
	...
	const { createElement: el$1, useState, useEffect } = wp.element;
	const S_ERROR = {
		margin: "4px 0 0",
		fontSize: "12px",
		color: "#cc1818"
	...
	const textLink = (label, onClick, style) => el$1("a", {
		href: "#",
		onClick: (e) => {
			e.preventDefault();
			onClick();
	...
	const DrptTokenPanel = ({ postId, Btn, SelectInput, onBeforeOpenPreview }) => {
		...
		const expirySelector = () => el$1("div", null, el$1(SelectInput, {
			label: t().expiry,
			value: preset,
			options: PRESET_OPTIONS,
			onChange: handlePresetChange
		}), preset === "custom" ? el$1("input", {
			key: "custom-dt",
			type: "datetime-local",
			value: customIso,
		...
		if (isActive) {
			...
			}))), mode === "confirm_delete" ? el$1("span", { style: { fontSize: "12px" } }, t().deleteConfirm + " ", el$1(Btn, {
				variant: "link",
				isDestructive: true,
				isSmall: true,
				onClick: doDelete,
